...
// Búsqueda por nombre (si hay)
$filtro = "";
if (isset($_GET['buscar']) && !empty(trim($_GET['buscar']))) {
    $filtro = trim($_GET['buscar']);
    $query = "SELECT * FROM abogado WHERE nombre LIKE ?";
    $stmt = mysqli_prepare($conexion, $query);
    $buscar = "%" . $filtro . "%";
    mysqli_stmt_bind_param($stmt, "s", $buscar);
} else {
    $query = "SELECT * FROM abogado ORDER BY apellido ASC";
    $stmt = mysqli_prepare($conexion, $query);
}
...
<body class="container mt-5">
    ...
    <form class="mb-4" method="GET" action="">
        <div class="input-group">
            ...
            <a href="crear.php" class="btn btn-success ms-2">➕ Nuevo Abogado</a>
        </div>
    </form>
